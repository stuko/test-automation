...
public class TestMessage extends TestPluginMessageBuilder {
    ...
    public FileJsonArrayList getFileMessage(TestPluginCallBack callBack) {
        // List<Map<String,String>> result = new ArrayList<>();
        FileJsonArrayListPlus result = new FileJsonArrayListPlus("./fds/data/");
        ...
        for(TestPluginMessageFactorImplFactory factor: this.message.getBodys()){
            ...
            List<String> rangeValueList = factor.getFactorRange().getValues();
            FileJsonArrayListPlus tmp = new FileJsonArrayListPlus("./fds/data/");
            ...
            //int start = 1;
            FileJsonArrayListPlus tmp2 = new FileJsonArrayListPlus("./fds/data/");
            // 기존에 생성한 결과를 Loop
            tmp.forEach(s->{
                // 항목의 범위 만큼 Loop
                for (int rIdx = 0; rIdx < rangeValueList.size() ; rIdx++) {
                    ...
                    if(TestPluginConstants.fds_random_key_data.equals(rangeValueList.get(rIdx))) {
            ...
            // 결과 리스트를 신규 생성함.
            result.clear();
            result = new FileJsonArrayListPlus("./fds/data/");
