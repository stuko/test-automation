...
public class SubstringMessageFactorImpl extends BasicMessageFactorImpl implements TestPluginMessageSubstringFactor {
    ...
    public void checkSubstring(){
        if(this.getValue().trim().startsWith(TestPluginConstants.fds_substring_data)){
            if(this.getValue().trim().indexOf(" ") > 0) {
                ...
                if(substrs.length == 3) {
                    ...
                    String to = substrs[2];
                    if(from.startsWith(TestPluginConstants.fds_substring_from)
                            && to.startsWith(TestPluginConstants.fds_substring_to)){
